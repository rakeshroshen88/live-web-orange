	 ...
	 echo "<ul id='country-list'>";
		 $db1->query($sql1)or die($db1->error());
		  while($row1=$db1->fetchArray()){
			...
			$work=$db1->getSingleResult("select current_company from user_profile where user_id=".$row1['user_id']);
			
			$pimage=$db1->getSingleResult('select image_id from user_profile where user_id='.$row1['user_id']);	
			
			if(!empty($pimage)){
			$path='<img src="upload/'.$pimage.'" alt="" height="50" width="50">';
			}else{ 
			$path='<img src="images/resources/s2.png" alt="">';
			}								
