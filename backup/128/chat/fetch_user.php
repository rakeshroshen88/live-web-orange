...
foreach($result as $row)
{
	...
	<button type="button" class="start_chat" data-touserid="'.$row['user_id'].'" data-tousername="'.$row['name'].'">
		  <div class="chat-user-img"><img src="'.$upath.'"></div><div class="chat-user-name"><h3>'.$row['name'].' '.count_unseen_message($row['user_id'], $_SESSION['user_id'], $connect).' '.fetch_is_type_status($row['user_id'], $connect).'</h3></div>'.fetch_is_status($row['user_id'], $connect).'
		  </button>
	</li>';
}
