			...
			<div class="main-section-data">
				<div class="row">
					...
					<div class="col-lg-9">
						...
						<?php $cpath=$db->getSingleResult("select bimg from category where id='".$_GET['cid']."'"); ?>
						<img src="category/<?=$cpath?>"/>
						 ...
						 if(!empty($_GET['cid'])){
							 if($_GET['cid']=='all'){
							 $sql="SELECT * from ".$_TBL_PRODUCT.' and prod_status=1 order by id desc'; }else{
								 if(!empty($_GET['sid'])){
								  $sql="SELECT * from ".$_TBL_PRODUCT." where prod_status=1 and catid=".$_GET['cid']." and subcatid=".$_GET['sid'];
								 }else{
									$sql="SELECT * from ".$_TBL_PRODUCT." where prod_status=1 and catid=".$_GET['cid'];
								 }
							 }
						}else{
							 $sql="SELECT * from ".$_TBL_PRODUCT.' where prod_status=1 order by id desc';
						}
