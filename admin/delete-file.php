...
{
	// select image name
	$image=mysql_query("select image from $_TBL_ITEMIMAGE where id='".$_GET["imageid"]."'") or die(mysql_error());
	if(mysql_num_rows($image)>0)
	{
		$image_row=mysql_fetch_array($image);
		$image_name=$image_row["image"];
			
		$check=mysql_query("delete from $_TBL_ITEMIMAGE where id='".$_GET["imageid"]."'") or die(mysql_error());
