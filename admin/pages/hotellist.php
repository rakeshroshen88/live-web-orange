...
if($act=='dac')
	{
		if($stat==0)
			...
			$stat=0;
		 $sql="UPDATE $_TBL_HOTELLIST SET status= '$stat' WHERE id='$id'";
		$db->query($sql);
...
if($act=='del')
	{
		...
		$sql="DELETE FROM $_TBL_HOTELLIST WHERE id='$id'";
		$db->query($sql);
		redirect('main.php?mod=hotellist');
	}
                ...
                <td height="300" valign="top" class="right"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                  ...
                  <tr>
                    <td bgcolor="#E6E6E6" class="tdpad"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                      <tr>
                        <td bgcolor="#FFFFFF" class="titlebar-bg">hotellist Management</td>
                      </tr>
					  <tr>
					...
					<tr><td colspan="2" align="center"><input type="submit" value="Search" name="search"></td></tr>
                      ...
                      <tr>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
                          ...
                          <tr>
                            <td width="50%">
							<div style="width:200px; height:40px; text-align:center; background:#CF9;">
						<a href="main.php?mod=addhotellist&act=add">
						<b>hotellist </b></a></div></td>
...
$db1=new DB();
	...
	//if(!empty($_REQUEST['firstname']) and isset($_REQUEST['firstname']))
	//{		
	$sql="SELECT * from ".$_TBL_HOTELLIST.$wherestr.$order_field;
	//}else{
	//	$wherestr1=" where type='flight' order by id desc";
	//$sql="SELECT * from ".$_TBL_HOTELLIST.$wherestr1;
	//}
	$db->query($sql);
...
?>						  
						  <tr>
                            ...
                            <td valign="top" align="left"><a href="main.php?mod=addhotellist&act=edit&id=<?=$row['id']?>"><?=$row['title']?></a></td>
							<td valign="top" align="center"><?php echo date('d M,Y',$st);?></td>							
                           <td valign="top" align="center"><a href='main.php?<?=$qryStr?>&act=dac&id=<?=$row['id']?>&stat=<?=$row['status']?>'><?=$row['status']==0?'Deactive':'Active'?></a> </td>
                            <td align="center" valign="top"><a href="main.php?mod=addhotellist&act=edit&id=<?=$row['id']?>"><img height="16" alt="Edit USER" src="images/edit.png" width="16" border="0" /> &nbsp;<a href='javascript:deladmin("<?=$row['id']?>")'><img height="16" alt="Delete Category" src="images/delete.png" width="16" border="0" /></a>
							</td>
                          </tr>
<?php } ?>
