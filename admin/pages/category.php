...
if($act=='dac')
	{
		if($stat=='no')
			...
			$stat='no';
		$sql="UPDATE $_TBL_CAT SET status='$stat' WHERE id='$catid'";
		$db->query($sql);
...
if($act=='del')
	{
		...
		$sql="DELETE FROM $_TBL_CAT WHERE id='$catid'";
		$db->query($sql);
		redirect('main.php?mod=cat');
	}
...
function deladmin(id)
{
	...
	{
		location.href="<?=$_PAGE.'?'.$qryStr?>&mod=cat&act=del&id="+id;
	}
}
</script>
				...
				<div class="panel panel-blue panel-widget ">
					<div class="row no-padding">
						...
						<div class="col-sm-9 col-lg-7 widget-right">
							<div class="large"><?=$count=$db->getSingleResult("select count(*) from $_TBL_CAT");	?></div>
            ...
            <div class="col-xs-12 col-md-6 col-lg-3">
				<div class="panel panel-orange panel-widget">
					<div class="row no-padding">
						...
						<div class="col-sm-9 col-lg-7 widget-right">
						<div class="large"><?=$count=$db->getSingleResult("select count(*) from $_TBL_CAT where status='1'");	?></div>
...
$db1=new DB();
	...
	if(!empty($_REQUEST['firstname']) or isset($_REQUEST['firstname']))
	 ...
	 $sql="SELECT * from ".$_TBL_CAT.$wherestr." order by id desc";
	$db->query($sql);
...
?>	
 					<tr>
						<td></td>
                        <td> <a href="main.php?mod=add_cat&act=edit&id=<?=$row['id']?>"><?=$row['catname']?></a></td>
							
                   
						<td> <?php echo date('d M,Y',$st);?></td>
                    <td> <a href='main.php?mod=cat&act=dac&id=<?=$row['id']?>&stat=<?=$row['status']?>'><?=$row['status']=='no'?'Deactive':'Active'?></a> </td>
                           
                        <td > <a href="main.php?mod=add_cat&act=edit&id=<?=$row['id']?>"> <span class="glyphicon glyphicon-edit" title="Edit"></span> &nbsp;<a href='javascript:deladmin("<?=$row['id']?>")'> <span class="glyphicon glyphicon-trash" title="Delete"></span></a>
						</td>
                     </tr>
<?php } ?>
<?php	}else{
