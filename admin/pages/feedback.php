...
if($act=='del')
	{
		...
		$sql="DELETE FROM feedback WHERE id='$catid'";
		$db->query($sql);
		redirect('main.php?mod=feedback');
	}
...
function deladmin(id)
{
	...
	{
		location.href="<?=$_PAGE.'?'.$qryStr?>&mod=feedback&act=del&id="+id;
	}
}
</script>
				...
				<div class="panel panel-blue panel-widget ">
					<div class="row no-padding">
						...
						<div class="col-sm-9 col-lg-7 widget-right">
							<div class="large"><?=$count=$db->getSingleResult("select count(*) from feedback");	?></div>
            ...
            <div class="col-xs-12 col-md-6 col-lg-3">
				<div class="panel panel-orange panel-widget">
					<div class="row no-padding">
						...
						<div class="col-sm-9 col-lg-7 widget-right">
						<div class="large"><?=$count=$db->getSingleResult("select count(*) from feedback where status='1'");	?></div>
							<div class="text-muted">Active feedback</div>
						</div>
					</div>
				</div>
			</div>
...
$db1=new DB();
	...
	if(!empty($_REQUEST['firstname']) or isset($_REQUEST['firstname']))
	 ...
	 $sql="SELECT * from feedback".$wherestr." order by id desc";
	$db->query($sql);
...
?>	
 					<tr>
						<td></td>
                        <td> <a href="main.php?mod=add_feedback&act=edit&id=<?=$row['id']?>"><?=$row['title']?></a></td>
