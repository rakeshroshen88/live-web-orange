...
if($act=='dac')
	{
		if($stat==0)
			...
			$stat=0;
		$sql="UPDATE $_TBL_USER SET user_status= '$stat' WHERE id='$id'";
		$db->query($sql);
...
if($act=='del')
	{
		 ...
		 $sql="DELETE FROM $_TBL_USER WHERE id='$id'";
		$db->query($sql);
		redirect('main.php?mod=view');
	}
...
$db1=new DB();
	...
	if(!empty($_REQUEST['firstname']) or isset($_REQUEST['firstname']))
		{
		$wherestr=" where name like '%$firstname%'";
		}
		if(empty($rec))
			{
			$_LIST_LEN=10;
	...
	$sql="SELECT * from ".$_TBL_USER.$wherestr." order by id desc";
	$db->query($sql);
...
?>	
 					<tr>
						<td></td>
                        <td> <a href="main.php?mod=adduser&act=edit&id=<?=$row['id']?>"><?=$row['name']?></a></td>
							
                        <td> <?=$row['email']?></td>
						<td> <?php echo date('d M,Y',$st);?></td>
                    <td> <a href='main.php?mod=view&act=dac&id=<?=$row['id']?>&stat=<?=$row['user_status']?>'><?=$row['user_status']==0?'Deactive':'Active'?></a> </td>
                           
                        <td > <a href="main.php?mod=adduser&act=edit&id=<?=$row['id']?>"> <span class="glyphicon glyphicon-edit" title="Edit"></span> &nbsp;<a href='javascript:deladmin("<?=$row['id']?>")'> <span class="glyphicon glyphicon-trash" title="Delete"></span></a>
						</td>
                     </tr>
<?php } ?>
