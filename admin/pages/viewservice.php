...
if($act=='del')
	{
		 ...
		 $sql="DELETE FROM $_TBL_SERVICE WHERE id='$id'";
		$db->query($sql);
		redirect('main.php?mod=viewservice');
	}
...
?>	
                    ...
                    <td> <a href='main.php?mod=viewservice&act=dac&id=<?=$row['id']?>&stat=<?=$row['status']?>'><?=$row['status']=='no'?'Deactive':'Active'?></a> </td>
                        ...
                        <td > <a href="main.php?mod=addservice&act=edit&id=<?=$row['id']?>"> <span class="glyphicon glyphicon-edit" title="Edit"></span> &nbsp;<a href='javascript:deladmin("<?=$row['id']?>")'> <span class="glyphicon glyphicon-trash" title="Delete"></span></a>
						</td>
                     </tr>
<?php } ?>
<?php	}else{
