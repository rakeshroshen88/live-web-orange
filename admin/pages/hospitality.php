...
if($act=='dac')
	{
		if($stat=='no')
			...
			$stat='no';
		$sql="UPDATE $_TBL_HOSPITALITY SET status='$stat' WHERE id='$catid'";
		$db->query($sql);
...
if($act=='del')
	{
		...
		$sql="DELETE FROM $_TBL_HOSPITALITY WHERE id='$catid'";
		$db->query($sql);
		redirect('main.php?mod=hospitality');
	}
...
$db1=new DB();
	...
	if(!empty($_REQUEST['firstname']) or isset($_REQUEST['firstname']))
	 ...
	 $sql="SELECT * from ".$_TBL_HOSPITALITY.$wherestr." order by id desc";
	$db->query($sql);
