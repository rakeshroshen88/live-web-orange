...
if($act=='dac')
	{
		if($stat==0)
			...
			$stat=0;
		$sql="UPDATE $_TBL_STATE SET status= '$stat' WHERE id='$id'";
		$db->query($sql);
		redirect('main.php?mod=viewstate');
	}

if($act=='del')
	{
		
		 $sql="DELETE FROM $_TBL_STATE WHERE id='$id'";
		$db->query($sql);
		redirect('main.php?mod=viewstate');
	}
...
function deladmin(id)
{
	...
	{
		location.href="main.php?mod=viewstate&act=del&id="+id;
	}
}
</script>
...
$db1=new DB();
	...
	$sql="SELECT * from ".$_TBL_STATE.$wherestr." order by id desc";
	$db->query($sql);
...
?>	
 					<tr>
						<td></td>
                        <td> <a href="main.php?mod=addstate&act=edit&id=<?=$row['id']?>"><?=$row['title']?></a></td>
							
                        
						<td> <?php echo date('d M,Y',$st);?></td>
                    <td> <a href='main.php?mod=viewstate&act=dac&id=<?=$row['id']?>&stat=<?=$row['status']?>'><?=$row['status']==0?'Deactive':'Active'?></a> </td>
                           
                        <td > <a href="main.php?mod=addstate&act=edit&id=<?=$row['id']?>"> <span class="glyphicon glyphicon-edit" title="Edit"></span> &nbsp;<a href='javascript:deladmin("<?=$row['id']?>")'> <span class="glyphicon glyphicon-trash" title="Delete"></span></a>
						</td>
                     </tr>
<?php } ?>
