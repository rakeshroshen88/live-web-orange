                ...
                <td height="300" valign="top" class="right"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                  ...
                  <tr>
                    <td bgcolor="#E6E6E6" class="tdpad"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                      <tr>
                        <td bgcolor="#FFFFFF" class="titlebar-bg">Hotel Management</td>
                      </tr>
					  <tr>
					...
					<tr><td colspan="2" align="center"><input type="submit" value="Search" name="search"></td></tr>
                      ...
                      <tr>
						...
						<a href="main.php?mod=addhotel&act=add">
						<b>Add Hotal</b></a></div></td>
...
$db1=new DB();
	...
	if(!empty($_REQUEST['firstname']) and isset($_REQUEST['firstname']))
	{		
	$sql="SELECT * from ".$_TBL_HOTEL.$wherestr.$order_field;
	}else{
		$wherestr1=" where status='1' order by id desc";
	 $sql="SELECT * from ".$_TBL_HOTEL.$wherestr1;
	}
	$db->query($sql);
...
?>						  
						  <tr>
                            ...
                            <td valign="top" align="left"><a href="main.php?mod=addhotel&act=edit&id=<?=$row['id']?>"><?=$row['title']?></a></td>
							<td valign="top" align="center"><?php echo date('d M,Y',$st);?></td>							
                           <td valign="top" align="center"><a href='main.php?act=dac&id=<?=$row['id']?>&stat=<?=$row['status']?>'><?=$row['status']==0?'Deactive':'Active'?></a> </td>
                            <td align="center" valign="top"><a href="main.php?mod=addhotel&act=edit&id=<?=$row['id']?>"><img height="16" alt="Edit USER" src="images/edit.png" width="16" border="0" /> &nbsp;<a href='javascript:deladmin("<?=$row['id']?>")'><img height="16" alt="Delete Category" src="images/delete.png" width="16" border="0" /></a>
							</td>
                          </tr>
<?php } ?>
