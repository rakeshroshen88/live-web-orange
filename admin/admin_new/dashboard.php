...
<div class="posts-section" id="postshow"></div>
...
<div class="app-timeline-item">
	        ...
	        <div class="content">
				...
				<div class="job_descp"><h3 class="font-weight-500"><?php if (!empty($row['post_title']))
					{
					    echo $row['post_title'];
					}
					if (!empty($feelingimgid))
					{ ?>: <img src="allimg/<?=$feelingimgidpath ?>"width="20"> <?=$feelingimgid ?> <span class="bold"> <?=$row['postemos'] ?> </span> <?php
					} ?></h3> <p><?=$row['post_details'] ?> </p> <?php $ext = pathinfo($row['allpath'], PATHINFO_EXTENSION);
										...
										$u=$dbmost->getSingleResult("select user_id from followers where follow=".$mostrow['user_id']);
											...
											?>
												...
												<?php if($mostrow['image_id']){?>
													<a href="view-profile.php?uid=<?php echo base64_encode($mostrow['user_id']);?>"> <img src="upload/<?=$mostrow['image_id']?>" alt="" height="100" width="100" style="    margin-left: 30px;"></a>
                                                   <?php }else{ ?>
                                                <a href="view-profile.php?uid=<?php echo base64_encode($mostrow['user_id']);?>"><img src="images/resources/user.png" alt="" width="100" height="100" style="    margin-left: 30px;"></a>
...
$mainuserimage=$db1->getSingleResult('select image_id from user_profile where user_id='.$_SESSION['sess_webid']);
  ...
  $sqlp="SELECT * from user_post where FIND_IN_SET(".$_SESSION['sess_webid'].",tagfriends) or user_id='".$_SESSION['sess_webid']."' or user_id IN($allfriends) or user_id IN($allfriend1) and post_hide='0' order by post_id desc limit 5,50";
$dbp->query($sqlp);
if($dbp->numRows()>0)
{
while($row=$dbp->fetchArray()){
