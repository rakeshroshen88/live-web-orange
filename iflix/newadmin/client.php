...
function deladmin(id)
{
	...
	{
		location.href="https://iflex.ng/admin/admin_new/client.php?act=del&aid=<?=$_SESSION['SES_ADMIN_ID']?>&id="+id;
	}
}
</script>
...
?>             <tr>
						<td><?=$ct?></td>
                        <td> <a href="//iflex.ng/admin/admin_new/add_all_user.php?act=edit&aid=<?=$_SESSION['SES_ADMIN_ID']?>&id=<?=$row['id']?>"><?=$row['adminname']?></a></td>
							
                        <td> <?=$row['adminemail']?></td>
						<td> <?php echo date('d M,Y',$st);?></td>
                    <td> <a href='//iflex.ng/admin/admin_new/client.php?act=dac&id=<?=$row['id']?>&aid=<?=$_SESSION['SES_ADMIN_ID']?>&stat=<?=$row['adminstatus']?>'><?=$row['adminstatus']==0?'Deactive':'Active'?></a> </td>
                           
                        <td > <a href="//iflex.ng/admin/admin_new/restaurants_list.php?act=edit&aid=<?=$_SESSION['SES_ADMIN_ID']?>&uid=<?=$row['id']?>"> <span class="glyphicon glyphicon-edit" title="Edit"></span> &nbsp;<a href='javascript:deladmin("<?=$row['id']?>")'> <span class="glyphicon glyphicon-trash" title="Delete"></span>						</a>
						</td>
                     </tr>
