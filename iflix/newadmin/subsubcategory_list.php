...
function deladmin(id)
{
	...
	{
		location.href="https://iflex.ng/admin/admin_new/subsubcategory_list.php?act=del&aid=<?=$_SESSION['SES_ADMIN_ID']?>&id="+id;
	}
}
</script>
                            ...
                            <div class="block-content">
						       ...
						       <th >Category </th>
                                ...
                                <th>Action</th>
                                    ...
                                    <tbody>
	<?php	$db1=new DB();
	$sql="SELECT * from ".$_TBL_SUBSUBCAT." order by id desc";
	$db->query($sql);
	if($db->numRows()>0)
		{
			$ct=1;
...
$st=mktime(0,0,0,$date[1],$date[2],$date[0]);			
	...
	<tr>
						<td><?=$ct?></td>
                        <td> <a href="https://iflex.ng/admin/admin_new/add_subsubcategory.php?act=edit&aid=<?=$_SESSION['SES_ADMIN_ID']?>&id=<?=$row['id']?>"><?=$row['subsubcatname']?></a></td>
							<td><?=$cat?></td>
<td><?=$subcatname?></td>
                   
						<td> <?php echo date('d M,Y',$st);?></td>
                    <td> <a href='https://iflex.ng/admin/admin_new/subsubcategory_list.php?act=dac&id=<?=$row['id']?>&aid=<?=$_SESSION['SES_ADMIN_ID']?>&stat=<?=$row['sub_status']?>'><?=$row['sub_status']=='0'?'Deactive':'Active'?></a> </td>
                           
                        <td > <a href="https://iflex.ng/admin/admin_new/add_subsubcategory.php?act=edit&id=<?=$row['id']?>&aid=<?=$_SESSION['SES_ADMIN_ID']?>"> <span class="glyphicon glyphicon-edit" title="Edit"></span> &nbsp;<a href='javascript:deladmin("<?=$row['id']?>")'> <span class="glyphicon glyphicon-trash" title="Delete"></span></a>
						</td>
                     </tr>
