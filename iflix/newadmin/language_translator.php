...
<div class="tranboxtranslator">
									<textarea name="tlanguage" id="tlanguage" placeholder="Type your Text !" rows="2" cols="30"style="border-radius: 5px; font-size: 22px; height: 100px;"></textarea>
...
<div class="tranboxtranslator">
									<textarea rows="2" cols="30" name="rlanguage" id="rlanguage" placeholder="Read Translation Here..." readonly  style="border-radius: 5px; font-size: 22px; height: 100px;"></textarea>
									</br>
									<div class="translate_control-a">
...
function clone() {
  ...
  alert("Clone the text: " + copyText.value);
                    ...
                    <div class="col-md-12" style="background-color: #ff5e00; height:105px; color:white; padding: 2px; margin-top: 15px; border-radius: 5px;">
				<?php	 $sql="SELECT * FROM slider_add WHERE img_type='Language' order by rand() limit 0,1";
		$db->query($sql)or die($db->error());
		$row=$db->fetchArray();
if(!empty($row['picture'])){		?>
		<a href="http://<?=$row['addlink']?>" target="_blank"><img src="slider/<?=$row['picture']?>" width="100%" height="100px" /></a>
<?php } ?>
					</div>
