...
if($act=='dac')
	{
		if($stat=='no')
			...
			$stat='no';
		$sql="UPDATE $_TBL_EVENT SET status= '$stat' WHERE eid='$id'";
		$db->query($sql);
...
if($act=='del')
	{
		...
		$sql="DELETE FROM hotel_rooms WHERE eid='$id'";
		$db->query($sql);
...
<?php
$ct=1;
 $sql="SELECT * from $_TBL_EVENT order by eid desc";
	$db->query($sql);
...
<tr>
						<td><?=$ct?></td>
                        <td> <a href="//iflex.ng/admin/admin_new/add_event.php?act=edit&aid=<?=$_SESSION['SES_ADMIN_ID']?>&id=<?=$row['eid']?>"><?=$row['etitle']?></a></td>
							
                       <td> <?=$row['price']?>.00</td>
						<td> <?php echo date('d M,Y',$st);?></td>
                    <td> <a href='//iflex.ng/admin/admin_new/event_list.php?act=dac&aid=<?=$_SESSION['SES_ADMIN_ID']?>&id=<?=$row['eid']?>&stat=<?=$row['status']?>'><?=$row['status']=='no'?'Deactive':'Active'?></a> </td>
                           
                        <td > <a href="//iflex.ng/admin/admin_new/add_rooms.php?act=edit&aid=<?=$_SESSION['SES_ADMIN_ID']?>&id=<?=$row['eid']?>"> <span class="glyphicon glyphicon-edit" title="Edit"></span> &nbsp;<a href='javascript:deladmin("<?=$row['eid']?>")'> <span class="glyphicon glyphicon-trash" title="Delete"></span>						</a>
						</td>
                     </tr>
