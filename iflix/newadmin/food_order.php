                      ...
                      <div class="block block-condensed">
                            ...
                            <div class="block-content">
                                ...
                                <table class="table table-striped table-bordered datatable-extended" id="sortable-data">
									...
									$db->query($sql);	if($db->numRows()>0)	{	while($row=$db->fetchArray()){	$num=$db1->getSingleResult('select count(*) from '.$_TBL_ORDER." where id=".$row['id']);	 $name=$db1->getSingleResult('select first_name from all_user where user_id='.$row['userid']);			$arr1=@explode(' ',$row['buydate']);		$edate=@explode('-',$arr1[0]);		$stamp1=@mktime(0,0,0,$edate[2],$edate[1],$edate[0]);		if($row['order_status']=="0")	{	$sta="<b style='color:red;'>Pending</b>";	}elseif($row['order_status']=="1"){		$sta="Cancelled";	}elseif($row['order_status']=="2"){		$sta="Confirmed";	}			?>							<tr>						<td><?=$ct?></td>                         <td><a href="//iflex.ng/admin/admin_new/food_order_details.php?id=<?=$row[orderid]?>&aid=<?=$_SESSION['SES_ADMIN_ID']?>"><?=$row['orderid']?></a></td>	
<td><?=$row['order_type']?></td>

									<td ><?=date('dS'.' '.'M',$stamp1)?><?=date('Y',$stamp1)?></td>						 <td ><?php echo $name;?></td>	<td align='center'>₦ <?=$row['totalprice']?></td>                           																																			                           <td>					   <?php echo $sta;?>							  </td>                        							 <td><a href="//iflex.ng/admin/admin_new/food_order_details.php?id=<?=$row['orderid']?>&aid=<?=$_SESSION['SES_ADMIN_ID']?>"> <span class="glyphicon glyphicon-edit" title="Edit"></span> &nbsp;
