 ...
 if($db->numRows()>0)
  {
	...
	{		
		...
		$prodcolor.=$row['prodcolor'].',';
		 ...
		 $totalsoldproduct=$db1->getSingleResult("select soldproduct from product where id=".$row['prodid']);
		 $newtotalsoldproduct=$totalsoldproduct+$row['quantity'];
		 $sqlsold="UPDATE product SET soldproduct='".$newtotalsoldproduct."' WHERE id=".$row['prodid'];
		 $dbup->query($sqlsold);	
