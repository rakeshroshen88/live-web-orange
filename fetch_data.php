...
$uid=$_POST['uid'];
if($_POST['profile']=='view'){
 $sqlp="SELECT * from user_post where user_id='".$uid."' and post_hide='0' order by post_id desc limit ".$row.",".$rowperpage; 
  
  }else{
	   $sqlp="SELECT * from user_post where FIND_IN_SET(".$_SESSION['sess_webid'].",tagfriends) or user_id='".$_SESSION['sess_webid']."' or user_id IN($allfriends) or user_id IN($allfriend1) and post_hide='0' order by post_id desc limit ".$row.",".$rowperpage; 
  }
  
/*  $sqlp="SELECT * from user_post where post_hide='0' order by post_id desc limit ".$row.",".$rowperpage; */

 $allcount=$db1->getSingleResult("SELECT count(post_id) from user_post where FIND_IN_SET(".$_SESSION['sess_webid'].",tagfriends) or user_id='".$_SESSION['sess_webid']."' or user_id IN($allfriends) or user_id IN($allfriend1) and post_hide='0' order by post_id desc");
$dbp->query($sqlp);
if($dbp->numRows()>0)
{
...
$ccount=$dblike->getSingleResult('select count(c_id) from comment where post_id='.$row['post_id']);
 $id = $row['post_id'];
?>
<div class="app-timeline-item" id="post_<?php echo $id; ?>">
	        <div class="user">
				                        ...
				                        <div class="contact-container">
				                            ...
				                            <div class="replybody-mg">
					                            <?php if(!empty($rowc['mp3'])){ ?>
														...
														<img src="upload/<?=$rowc['cimage']?>" height="50" width="50"/></br>
														<?php }?>
						                    ...
						                    <div class="contact contact-rounded contact-lg">
						                        ...
						                        <div class="contact-container">
						                            ...
						                            <?php if(!empty($rowr['rimage'])){ ?></br>
																	...
																	<span class="commword"></br><?=$rowr['r_comment']?>
																		</span>																	
